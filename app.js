...
const fmt = {
  ...
  volume(v){ return (+v||0).toFixed(0)},
  weight(w){ return Math.round(+w||0)},
};
function escHtml(s){if(!s)return '';return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;')}
function cvtWt(v,fromUnit){const to=Settings.get('weightUnit');if(!fromUnit||fromUnit===to)return v;return fromUnit==='lbs'?v*0.453592:v*2.20462}
...
const Session = {
  ...
  async start(templateId){
    ...
    const currentUnit=Settings.get('weightUnit');
    if(templateId){
      const tpl=await DB.get('templates',templateId);
      if(tpl){
        for(const te of tpl.exercises){
          const ex=await DB.get('exercises',te.exerciseId);
          const prevData=this._getPreviousFrom(te.exerciseId,allSessions);
          const prev=prevData.sets;
          const sets=[];
          for(let i=0;i<(te.targetSets||3);i++){
            const w=prev[i]?Math.round(cvtWt(prev[i].weight,prevData.weightUnit)):te.targetWeight||0;
            sets.push({setNumber:i+1,weight:w,reps:prev[i]?.reps||te.targetReps||0,type:'normal',rpe:null,completed:false,completedAt:null});
          }
  ...
  _getPreviousFrom(exerciseId,sessions){
    const sorted=sessions.slice().sort((a,b)=>b.startedAt-a.startedAt);
    for(const s of sorted){
      const ex=s.exercises?.find(e=>e.exerciseId===exerciseId);
      if(ex)return {sets:ex.sets||[],weightUnit:s.weightUnit||null};
    }
    return {sets:[],weightUnit:null};
  },
  async addExercise(exerciseId){
    ...
    if(!ex)return;
    const allSessions=await DB.getAll('sessions');
    const prevData=this._getPreviousFrom(exerciseId,allSessions);
    const prev=prevData.sets;
    const sets=[];
    for(let i=0;i<3;i++){
      const w=prev[i]?Math.round(cvtWt(prev[i].weight,prevData.weightUnit)):0;
      sets.push({setNumber:i+1,weight:w,reps:prev[i]?.reps||0,type:'normal',rpe:null,completed:false,completedAt:null});
    }
  ...
  async finish(){
    ...
    const session={
      ...
      weightUnit:Settings.get('weightUnit'),
  ...
  async _checkPRs(session){
    const newPRs=[];
    const unit=session.weightUnit||Settings.get('weightUnit');
    for(const ex of session.exercises){
      ...
      for(const set of ex.sets){
        const oldWt=cvtWt(pr.maxWeight.value,pr.maxWeight.unit);
        const newWt=cvtWt(set.weight,unit);
        if(newWt>oldWt){pr.maxWeight={value:set.weight,unit,date:session.completedAt,sessionId:session.id};updated=true;newPRs.push({exerciseName:ex.exerciseName,type:'Weight',value:set.weight})}
        ...
        const oldVol=cvtWt(pr.maxVolume.value,pr.maxVolume.unit);
        const newVol=cvtWt(vol,unit);
        if(newVol>oldVol){pr.maxVolume={value:vol,unit,date:session.completedAt,sessionId:session.id};updated=true}
      }
...
function showTemplatePickerModal(templates){
  ...
  const programHtml=Object.entries(programs).map(([pid,p])=>`
    <div style="margin:8px 0">
      <div style="font-size:14px;font-weight:700;color:var(--text-primary);margin-bottom:4px">${escHtml(p.programName)}</div>
      ${p.days.map((d,i)=>`<div class="template-card" style="margin:4px 0;padding:10px 14px;display:flex;align-items:center;gap:8px" data-action="start-template" data-id="${d.id}">
        ...
        <div style="flex:1;min-width:0"><h3 style="font-size:14px;margin:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escHtml(d.dayLabel||d.name)||'Untitled'}</h3>
  ...
  const standaloneHtml=standalone.map(t=>`<div class="template-card" style="margin:8px 0" data-action="start-template" data-id="${t.id}">
    <h3>${escHtml(t.name)||'Untitled'}</h3>
    <div class="meta">${t.exercises?.length||0} exercises</div>
    <div style="margin-top:4px;display:flex;gap:4px;flex-wrap:wrap">
      ${(t.exercises||[]).slice(0,4).map(e=>`<span class="chip" style="font-size:10px;padding:2px 6px">${escHtml(e.exerciseName||e.exerciseId)}</span>`).join('')}
...
async function viewDashboard(){
  ...
  const totalVolumeWeek=thisWeek.reduce((a,s)=>a+cvtWt(s.totalVolume,s.weightUnit),0);
  const totalVolumeLast=lastWeek.reduce((a,s)=>a+cvtWt(s.totalVolume,s.weightUnit),0);
        ...
        if(programDays.length>1){
          ...
          nextWorkoutHtml=`<div class="card" style="border-color:var(--accent);cursor:pointer;margin-top:8px" onclick="Session.start('${nextDay.id}')">
            <div class="flex items-center gap-8">
              ...
              <div style="flex:1"><strong>Next: ${escHtml(nextDay.dayLabel||nextDay.name)}</strong>
              <div class="text-muted" style="font-size:13px">${escHtml(lastTpl.programName)||'Program'} &middot; ${nextDay.exercises?.length||0} exercises</div></div>
  ...
  renderApp(`
    ...
    <div class="stats-grid mt-16">
      ...
      <div class="stat-card"><div class="stat-value">${fmt.volume(totalVolumeWeek)}${arrowVol(wowVolume,unit)}</div><div class="stat-label">Volume (${unit})</div></div>
    ...
    ${recent.map(s=>`<div class="history-item" onclick="Router.navigate('history/${s.id}')">
      <div style="flex:1"><strong>${escHtml(s.name)||'Workout'}</strong><div class="history-date">${fmt.date(s.completedAt)} &middot; ${fmt.duration(s.duration)} &middot; ${s.totalSets} sets</div></div>
      <span style="color:var(--text-muted)">${icons.chevronRight}</span>
    </div>`).join('')}
    ...
    ${(await Promise.all(recentPRs.map(async pr=>{let ex=EXERCISES.find(e=>e.id===pr.exerciseId);if(!ex)ex=await DB.get('exercises',pr.exerciseId);return `<div class="card pr-glow" style="border-color:var(--accent)">
      <div class="flex items-center gap-8">${icons.trophy}<div><strong>${escHtml(ex?.name)||escHtml(pr.exerciseId)}</strong>
      <div class="text-muted" style="font-size:13px">Weight: ${fmt.weight(cvtWt(pr.maxWeight?.value||0,pr.maxWeight?.unit))}${unit} &middot; Reps: ${pr.maxReps?.value||0}</div></div></div>
    </div>`}))).join('')}
    `:''}
  `);
...
function renderExerciseCard(ex){
  ...
  return `<div class="exercise-img-card" onclick="Router.navigate('library/${ex.id}')">
    ${ex.imageUrl?`<img class="exercise-img" src="${ex.imageUrl}" alt="${escHtml(ex.name)}" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">`:''}
    <div class="exercise-icon" style="background:${color};${ex.imageUrl?'display:none':''}">${escHtml(ex.name[0])}</div>
    <div style="flex:1"><h3>${escHtml(ex.name)}</h3><div class="sub">${escHtml(ex.muscleGroup)} &middot; ${escHtml(ex.equipment)}</div></div>
    <span style="color:var(--text-muted)">${icons.chevronRight}</span>
  </div>`;
}
...
async function viewExternalExerciseList(muscleGroup) {
  ...
  renderApp(`
    <div class="app-header">
      ...
      <h1>${escHtml(muscleGroup)}</h1>
    </div>
    <div class="search-bar"><input class="input" id="ext-search" placeholder="Search ${escHtml(muscleGroup.toLowerCase())} exercises..." autocomplete="off"/></div>
  ...
  function renderList() {
    ...
    list.innerHTML = filtered.length ? filtered.map(e => `<div class="ext-ex-card ${selected.has(e.id) ? 'selected' : ''}" data-id="${e.id}">
      ...
      <div class="ext-ex-info">
        ...
        <div class="sub">${escHtml(e.equipment)}${e.secondaryMuscles.length ? ' &middot; ' + e.secondaryMuscles.map(m=>escHtml(m)).join(', ') : ''}</div>
      </div>
    </div>`).join('') : '<div class="empty-state" style="padding:32px"><p>All exercises already in your library!</p></div>';
  }
...
async function viewExerciseDetail(id){
  ...
  renderApp(`
    ...
    ${ex.images&&ex.images.length>0?`<div style="display:flex;gap:8px;padding:16px;justify-content:center;background:var(--bg-surface)">
      ${ex.images.map(img=>`<img src="${img}" alt="${escHtml(ex.name)}" loading="lazy" style="max-height:220px;max-width:48%;object-fit:contain;border-radius:var(--radius);background:var(--bg-surface)" onerror="this.style.display='none'"/>`).join('')}
    </div>`:ex.imageUrl?`<div style="text-align:center;padding:16px;background:var(--bg-surface)">
      <img src="${ex.imageUrl}" alt="${escHtml(ex.name)}" loading="lazy" style="max-height:240px;object-fit:contain;border-radius:var(--radius)" onerror="this.style.display='none'"/>
    </div>`:`<div class="exercise-svg-container">${SVGAnimations.get(ex.animationId,ex.muscleGroup)}</div>`}
    <div class="card">
      <div style="font-size:14px;color:var(--text-secondary)">${escHtml(ex.description)}</div>
      <div class="flex gap-8 mt-8" style="flex-wrap:wrap">
        <span class="chip" style="background:${muscleColor(ex.muscleGroup)};color:#fff;border:none">${escHtml(ex.muscleGroup)}</span>
        ${ex.secondaryMuscles.map(m=>`<span class="chip">${escHtml(m)}</span>`).join('')}
        <span class="chip">${escHtml(ex.equipment)}</span>
        <span class="chip">${escHtml(ex.category)}</span>
      </div>
    </div>
    ${ex.tips?.length?`<div class="card">
      <div style="font-weight:600;margin-bottom:8px">Tips</div>
      <ul style="padding-left:20px;color:var(--text-secondary);font-size:14px">${ex.tips.map(t=>`<li style="margin-bottom:4px">${escHtml(t)}</li>`).join('')}</ul>
    </div>`:''}
    ${pr?`<div class="card pr-glow" style="border-color:var(--accent)">
      ...
      <div class="stats-grid" style="padding:0">
        <div class="stat-card"><div class="stat-value">${fmt.weight(cvtWt(pr.maxWeight?.value||0,pr.maxWeight?.unit))}<small>${unit}</small></div><div class="stat-label">Best Weight</div></div>
    ...
    ${history.map(s=>{const exData=s.exercises.find(e=>e.exerciseId===id);
      ...
      <div style="font-size:13px;color:var(--text-secondary);margin-top:4px">${exData.sets.map(st=>`${fmt.weight(cvtWt(st.weight,s.weightUnit))}${unit}x${st.reps}`).join(', ')}</div></div>`}).join('')}
    `:''}
  `);
}
  ...
  const programHtml=Object.entries(programs).map(([pid,p])=>`
    <div class="program-card">
      ...
      ${p.days.map((d,i)=>`<div class="program-day">
        <span class="program-day-num">Day ${i+1}</span>
        <span class="program-day-label">${escHtml(d.dayLabel||d.name)||'Untitled'}</span>
  ...
  const standaloneHtml=standalone.map(t=>`
    <div class="template-card" style="cursor:default">
      ...
      <div style="margin-top:8px;display:flex;gap:4px;flex-wrap:wrap">
        ${(t.exercises||[]).map(e=>`<span class="chip" style="font-size:11px;padding:3px 8px">${escHtml(e.exerciseName||e.exerciseId)}</span>`).join('')}
      </div>
      <div style="display:flex;gap:8px;margin-top:12px">
...
function SplitEditor({ exercises, allExercises, title, nameValue, showNameInput, onSave, onCancel, onDelete }) {
    ...
    app.innerHTML = `
      ...
      <div class="split-editor">
        ...
        <div class="split-right">
          ${showNameInput ? `<div class="split-right-header"><input class="input" id="split-name" placeholder="Routine name (e.g. Push Day)" value="${escHtml(nameValue)||''}"/></div>` : ''}
  ...
  function renderLeftCard(ex) {
    ...
    return `<div class="split-ex-card" draggable="true" data-exid="${ex.id}">
      ...
      <div class="exercise-icon" style="background:${muscleColor(ex.muscleGroup)}${hasImg?';display:none':''}">${escHtml(ex.name[0])}</div>
      <div style="flex:1;min-width:0"><div class="ex-name">${escHtml(ex.name)}</div><div class="ex-sub">${escHtml(ex.muscleGroup)} &middot; ${escHtml(ex.equipment)}</div></div>
  ...
  function renderRightItem(te, i) {
    ...
    return `<div class="routine-ex-item" data-idx="${i}" draggable="true">
      <div class="ex-header">
        ...
        <span class="ex-name">${escHtml(te.exerciseName || ex?.name || te.exerciseId)}</span>
  ...
  function showExerciseInfoDialog(ex) {
    ...
    if(ex.images&&ex.images.length>0){
      imageHTML=`<div class="ex-info-images">${ex.images.map(img=>`<img src="${img}" alt="${escHtml(ex.name)}" loading="lazy" onerror="this.style.display='none'"/>`).join('')}</div>`;
    } else if(ex.imageUrl){
      imageHTML=`<div class="ex-info-images"><img src="${ex.imageUrl}" alt="${escHtml(ex.name)}" loading="lazy" onerror="this.style.display='none'"/></div>`;
    } else {
    ...
    root.innerHTML=`<div class="modal-overlay fade-in"><div class="modal slide-up" style="padding:0;overflow:hidden">
      ...
      <div class="ex-info-body">
        ...
        ${ex.description?`<div class="desc">${escHtml(ex.description)}</div>`:''}
        ${ex.tips&&ex.tips.length?`<div style="font-weight:600;margin-bottom:6px">Tips</div><ul class="tips-list">${ex.tips.map(t=>`<li>${escHtml(t)}</li>`).join('')}</ul>`:''}
...
async function viewProgramEditor(progId){
  ...
  renderApp(`
    ...
    <div class="card">
      ...
      <input class="input" id="prog-name" placeholder="e.g. Push/Pull/Legs" value="${escHtml(ps.programName)||''}"/>
    </div>
    <div class="section-label">Days</div>
    ${ps.days.length?ps.days.map((d,i)=>`
      <div class="program-editor-day" data-dayidx="${i}">
        <div class="day-info">
          <input class="day-label-input" data-dayidx="${i}" placeholder="Day ${i+1} label" value="${escHtml(d.dayLabel)||''}"/>
  ...
  root.innerHTML=`<div class="modal-overlay fade-in"><div class="modal slide-up">
    ...
    <div id="picker-list" style="max-height:50vh;overflow-y:auto">
      ${exercises.map(ex=>`<div class="exercise-card" data-action="pick-ex" data-id="${ex.id}">
        <div class="exercise-icon" style="background:${muscleColor(ex.muscleGroup)}">${escHtml(ex.name[0])}</div>
        <div><h3>${escHtml(ex.name)}</h3><div class="sub">${escHtml(ex.muscleGroup)} &middot; ${escHtml(ex.equipment)}</div></div>
      </div>`).join('')}
    </div>
  </div></div>`;
...
async function renderSession(){
  ...
  app.innerHTML=`
    <div class="session-header">
      <div style="flex:1"><strong>${escHtml(data.name)||'Workout'}</strong><div class="session-timer text-muted" style="font-size:13px;font-family:var(--font-mono)">${elapsed}</div></div>
      ...
      <div class="session-ex-tabs">
        ${data.exercises.map((ex,ei)=>{
          ...
          return `<button class="session-ex-tab ${ei===_activeExIdx?'active':''} ${done?'has-completed':partial?'has-partial':''}" data-action="switch-tab" data-idx="${ei}">
            <span>${escHtml(ex.exerciseName)}</span>
      ...
      ${activeEx?`<div class="session-ex-view">
        <div class="session-media">
          ${exInfo&&exInfo.images&&exInfo.images.length>0?`<div style="display:flex;gap:8px;justify-content:center">
            ${exInfo.images.map(img=>`<img src="${img}" alt="${escHtml(activeEx.exerciseName)}" loading="lazy" onerror="this.style.display='none'"/>`).join('')}
          </div>`:exInfo&&exInfo.imageUrl?`<img src="${exInfo.imageUrl}" alt="${escHtml(activeEx.exerciseName)}" loading="lazy" onerror="this.style.display='none'"/>`:
          `<div class="exercise-svg-container" style="margin:0">${SVGAnimations.get(exInfo?.animationId,exInfo?.muscleGroup)}</div>`}
          <div class="ex-desc">${escHtml(exInfo?.description)||''}</div>
          ${exInfo?.tips?.length?`<ul class="ex-tips">${exInfo.tips.map(t=>`<li>${escHtml(t)}</li>`).join('')}</ul>`:''}
          <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:4px">
            <span class="chip" style="font-size:11px;padding:3px 8px;background:${muscleColor(exInfo?.muscleGroup||'Full Body')};color:#fff;border:none">${escHtml(exInfo?.muscleGroup)||''}</span>
            ${(exInfo?.secondaryMuscles||[]).map(m=>`<span class="chip" style="font-size:11px;padding:3px 8px">${escHtml(m)}</span>`).join('')}
          </div>
        </div>
        <div class="session-sets-panel">
          <div class="flex items-center" style="justify-content:space-between;margin-bottom:12px">
            <h3 style="font-size:16px;font-weight:700">${escHtml(activeEx.exerciseName)}</h3>
...
async function loadPreviousData(data,unit){
  ...
  for(let ei=0;ei<data.exercises.length;ei++){
    const prevData=Session._getPreviousFrom(data.exercises[ei].exerciseId,allSessions);
    const prev=prevData.sets;
    const rows=$$(`tr[data-ei="${ei}"] .prev-col`);
    rows.forEach((td,si)=>{
      if(prev[si]){td.textContent=`${fmt.weight(cvtWt(prev[si].weight,prevData.weightUnit))}x${prev[si].reps}`}
    });
  }
}
  ...
  renderApp(`
    ...
    ${sorted.length?sorted.map(s=>`
      <div class="history-item" onclick="Router.navigate('history/${s.id}')">
        <div style="flex:1">
          <strong>${escHtml(s.name)||'Workout'}</strong>
          <div class="history-date">${fmt.dateFull(s.completedAt)}</div>
          <div style="font-size:13px;color:var(--text-secondary)">${s.totalSets} sets &middot; ${fmt.volume(cvtWt(s.totalVolume,s.weightUnit))} ${unit} &middot; ${fmt.duration(s.duration)}</div>
        </div>
        <span style="color:var(--text-muted)">${icons.chevronRight}</span>
      </div>
...
async function viewHistoryDetail(id){
  ...
  renderApp(`
    <div class="app-header">
      ...
      <h1>${escHtml(session.name)||'Workout'}</h1>
    </div>
    <div class="card">
      ...
      <div class="stats-grid" style="padding:0">
        ...
        <div class="stat-card"><div class="stat-value">${fmt.volume(cvtWt(session.totalVolume,session.weightUnit))}</div><div class="stat-label">Volume (${unit})</div></div>
    ...
    ${(await Promise.all(session.exercises.map(async ex=>{
      ...
      return `<div class="card">
        <div class="flex items-center gap-8 mb-8">
          <div class="exercise-icon" style="background:${color};width:32px;height:32px;font-size:14px">${escHtml((ex.exerciseName||'E')[0])}</div>
          <strong>${escHtml(ex.exerciseName)}</strong>
        </div>
        <table class="set-table">
          <thead><tr><th>SET</th><th>${unit.toUpperCase()}</th><th>REPS</th></tr></thead>
          <tbody>${ex.sets.map(s=>`<tr><td class="set-num">${s.setNumber}</td><td class="mono">${fmt.weight(cvtWt(s.weight,session.weightUnit))}</td><td class="mono">${s.reps}</td></tr>`).join('')}</tbody>
        </table>
      </div>`;
    }))).join('')}
...
async function repeatWorkout(sessionId){
  ...
  for(const e of session.exercises){
    const prevData=Session._getPreviousFrom(e.exerciseId,allSessions);
    const prev=prevData.sets;
    const sets=[];
    for(let i=0;i<e.sets.length;i++){
      const w=prev[i]?Math.round(cvtWt(prev[i].weight,prevData.weightUnit)):e.sets[i]?Math.round(cvtWt(e.sets[i].weight,session.weightUnit)):0;
      sets.push({setNumber:i+1,weight:w,reps:prev[i]?.reps||e.sets[i]?.reps||0,type:'normal',rpe:null,completed:false,completedAt:null});
    }
